...
export default function Billing() {
  ...
  const { bills, patients, adjustPayment, createBill, EMI_MIN, EMI_MAX } = useData();
  ...
  return (
    <>
          ...
          <table className="w-full min-w-[640px] text-left">
            ...
            <tbody className="divide-y divide-line">
                  ...
                  <Fragment key={b.id}>
                    <tr className="hover:bg-paper-dim/30 transition-colors">
                      ...
                      <td className="px-5 py-3.5">
                        ...
                        >
                          Payment {isOpen ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                        </button>
                      </td>
                    </tr>
                    {isOpen && (
              ...
              {filteredBills.length === 0 && (
                <tr><td colSpan={8} className="px-5 py-10 text-center text-[13px] text-slate-soft">No bills match "{query}".</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title="Create new bill">
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <div>
            <label className="block text-[12.5px] font-medium text-slate mb-1.5">Bill amount (Rs)</label>
            <input
              type="number"
              required
              min="1"
